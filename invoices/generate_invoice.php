...
if ($orderId) {
    redirect(SITE_URL . '/invoice.php?id=' . $orderId);
}
...
if (!$data) {
    setFlash('error', 'Invalid invoice request.');
...
$invNo = 'INV-' . strtoupper($data['plan_name']) . '-' . str_pad($data['id'], 5, '0', STR_PAD_LEFT);
$statusLabel = $data['status'] === 'active' ? 'Active' : ($data['status'] === 'pending' ? 'Pending' : 'Expired');

$pageTitle = 'Invoice - ' . $invNo;
...
<div class="invoice-wrapper">
    <div class="invoice-header">
        ...
        <div class="inv-right">
            <h2>INVOICE</h2>
            <div class="ref"><?php echo sanitize($invNo); ?></div>
        </div>
    </div>
    ...
    <div class="invoice-body">
        <div class="info-row">
            <div class="info-col">
                <h3>Bill To</h3>
                <p><strong><?php echo sanitize($data['developer_name']); ?></strong></p>
                <p class="small"><?php echo sanitize($data['developer_email']); ?></p>
                <?php if (!empty($gst)): ?>
            ...
            <div class="info-col right">
                ...
                <p class="small">Date: <?php echo date('M d, Y', strtotime($data['purchase_date'])); ?></p>
                <p class="small">Invoice: <?php echo sanitize($invNo); ?></p>
        ...
        <table class="invoice-table">
            ...
            <tbody>
                <tr>
                    <td><?php echo ucfirst($data['plan_name']); ?> Plan Subscription</td>
                    <td>1</td>
                    <td><?php echo $currency; ?><?php echo number_format($planPrice, 2); ?></td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="totals-wrap">
            <div class="totals-box">
                <div class="tr">
                    <span>Subtotal</span>
                    <span><?php echo $currency; ?><?php echo number_format($planPrice, 2); ?></span>
                </div>
                <?php if ($taxPercent > 0): ?>
                <div class="tr">
                    <span><?php echo sanitize($taxType); ?> (<?php echo $taxPercent; ?>%)</span>
    ...
    <div class="download-bar no-print">
        <a href="generate_invoice_pdf.php?id=<?php echo $data['id']; ?>" class="btn-download">
