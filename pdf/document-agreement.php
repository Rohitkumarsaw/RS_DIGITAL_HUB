...
// Get template
$template = getDocumentTemplate('agreement');
if (!$template) {
    die('Agreement template not found.');
}
...
$doc = new DocumentPdf();
try {
    $pdf = $doc->generateAgreement($developer, $bodyContent, $planName, $joinDate);
    $filename = 'agreement-' . $developer['registration_no'] . '-' . date('Ymd') . '.pdf';
    $doc->output($filename);
} catch (Exception $e) {
